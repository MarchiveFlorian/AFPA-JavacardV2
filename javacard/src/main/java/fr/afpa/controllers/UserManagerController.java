...
public class UserManagerController {
    ...
    private void save(ActionEvent e) {
        ...
        if (valid) {
            ...
            if (isNewContact == true) {
                ...
                dao.insert(selectedContact);
                isNewContact = false;
            } else {
                dao.update(selectedContact);
            }
    ...
    void delete(ActionEvent e) {
        ...
        if (!selectedContacts.isEmpty()) {
            // Liste pour stocker les contacts à supprimer
            List<Contact> contactsToRemove = new ArrayList<>(selectedContacts);

            // Créer une instance de ContactDAO
            ContactDAO dao = new ContactDAO();

            // Supprimer chaque contact par ID
            for (Contact contact : contactsToRemove) {
                boolean success = dao.delete(contact.getId()); // Vérifiez le succès de la suppression
                if (!success) {
                    System.out.println("Échec de la suppression pour l'ID: " + contact.getId());
                }
            }

            // Mettre à jour la liste des contacts dans votre tableau
